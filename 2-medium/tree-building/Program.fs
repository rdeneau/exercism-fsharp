...
type AllocatedColumnProvider() =
    ...
    interface IColumnProvider with
        member _.GetColumns(summary: Summary) =
            seq {
...
let main _ =
    ...
    let benchmarkConfig = manualConfig.AddColumnProvider(AllocatedColumnProvider()).AddDiagnoser(MemoryDiagnoser.Default).AddLogger(ConsoleLogger.Default);
